...
\usepackage{mathpazo} % Adds a bookish Palatino font
...
% Title
\title{
    \textbf{Progress Report: Building Foundation Models for Urban Mobility and Smart Cities} \\
    \large{Sub-Project: Enhancing Time-Series Foundation Models with Spatial Graph Adapters}
}
\author{
    Swapnil Aggarwal (SID: 26040) \\
    \textit{M.Tech in Smart Mobility and Logistics Systems} \\
    \textit{CiSTUP, Indian Institute of Science (IISc)} \\
    \texttt{swapnila@iisc.ac.in} \\
    \vspace{0.3cm}
    Under the supervision of \textbf{Dr. Pandarasamy Arjunan} \\
    \textit{Energy Informatics Lab, IISc}
}
\date{July 2026}
...
State-of-the-art spatio-temporal models such as STGCN, DCRNN, and Graph WaveNet train \textit{all} model parameters---both temporal and spatial---from scratch on each target dataset. While effective, this approach has significant drawbacks:
\begin{itemize}[nosep]
...
\hl{\textit{Can a frozen, general-purpose time-series foundation model be enhanced with a lightweight spatial adapter to approach the accuracy of fully-trained spatio-temporal models?}}
...
\textbf{STGCN} combines graph convolutions with gated temporal convolutions in a sandwich architecture. \textbf{DCRNN} models traffic flow as a diffusion process on the road graph using a sequence-to-sequence architecture with scheduled sampling. \textbf{Graph WaveNet} introduces an adaptive adjacency matrix learned jointly with dilated causal convolutions. \textbf{GMAN} uses spatial-temporal attention mechanisms. \textbf{AGCRN} proposes adaptive graph convolutional recurrent networks. These methods achieve state-of-the-art accuracy but require training all parameters from scratch on each dataset.
...
The METR-LA dataset contains traffic speed readings from 207 loop detectors on the Los Angeles highway network over 4 months (March--June 2012) at 5-minute intervals, totaling 34,272 time steps. We follow the standard 70/10/20 train/validation/test split.
...
\begin{itemize}[nosep]
    ...
    \item \textbf{Model 4: TTM + GCN Adapter (Residual).} An advanced architecture variant, where the spatial graph network learns a residual correction on top of the frozen temporal foundation model's zero-shot prediction.
...
\begin{table}[H]
\centering
\caption{Performance comparison on METR-LA. Best results in \textbf{bold}. Our proposed model is highlighted. $^\dagger$Published results from original papers. $^\star$Task-specific trainable parameters (excluding frozen backbone).}
...
DCRNN$^\dagger$ & 2.77 & 5.38 & 7.3\% & 3.15 & 6.45 & 8.8\% & 3.60 & 7.60 & 10.5\% & $\sim$1.8M \\
Graph WaveNet$^\dagger$ & \textbf{2.69} & \textbf{5.15} & \textbf{6.9\%} & \textbf{3.07} & \textbf{6.22} & \textbf{8.4\%} & 3.53 & 7.37 & 10.0\% & $\sim$2.5M \\
...
Model 4: TTM + GCN + Residual & 3.58 & 6.47 & 9.7\% & 4.16 & 7.63 & 11.9\% & 4.86 & 8.92 & 14.9\% & 57K$^\star$ \\
\rowcolor{yellow!30} Model 5: Co-Fine-Tuned & 3.52 & 6.57 & 9.5\% & 4.07 & 7.69 & 11.7\% & 4.75 & 8.86 & 14.8\% & 156K$^\star$ \\
\bottomrule
\end{tabular}
}
\end{table}
...
Overall RMSE & 10.5135 & 10.3050 & $-0.21$ \\
Overall MAPE & 19.01\% & 18.63\% & $-0.38\%$ \\
60-min MAE & 4.8833 & 4.8582 & $-0.03$ \\
60-min RMSE & 8.9812 & 8.9205 & $-0.06$ \\
\bottomrule
\end{tabular}
\end{table}
...
To rigorously test the generalization capabilities of our proposed architecture, we conducted a zero-shot spatial transferability experiment. We directly transferred our \textbf{Co-Fine-Tuned (Model 5)} model trained exclusively on the METR-LA dataset (207 sensors in Los Angeles) to the unseen PEMS-BAY dataset (325 sensors in the San Francisco Bay Area). No retraining or fine-tuning was performed on the PEMS-BAY dataset.
...
\midrule
\textbf{Our Model (Zero-Shot)} & 2.07 & 5.04 & 6.03\% & 2.39 & 5.70 & 6.94\% & 2.81 & 6.49 & 8.15\% \\
\bottomrule
\end{tabular}
}
\end{table}

Remarkably, our model achieved a zero-shot MAE of \textbf{2.81} at 60 minutes on PEMS-BAY. While it understandably trails the fully-supervised deep learning models like Graph WaveNet (which were trained for hundreds of epochs on the PEMS-BAY data itself), \textbf{our zero-shot model successfully outperforms traditional baselines}. At the 60-minute horizon, our zero-shot model (MAE 2.81) comfortably beats the ARIMA baseline (MAE 3.38) and the Historical Average baseline (MAE 2.88) without ever seeing a single data point from PEMS-BAY during training. It is also surprisingly close to a fully-trained STGCN (MAE 2.49). This definitively proves the graph-agnostic robustness and instant transferability of our architecture.
...
\bibitem{dlg}
Y.~Ma and J.~Tang, \textit{Deep Learning on Graphs}. Cambridge University Press, 2021.
